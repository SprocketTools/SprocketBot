...
# try:
#     connection = await asyncpg.connect(user=SQLsettings["user"], password=SQLsettings["password"],
#                                  database=SQLdatabaseName, host=SQLsettings["host"], port=SQLsettings["port"])
...
sanitizeKeywords = ["@", "/", "invalid_tank", "DROP TABLE", ";"]
...
@bot.command()
async def databaseTest(ctx, *, arg):
    ...
    async with bot.pool.acquire() as connection:
        ...
        async with connection.transaction():
            ...
            value = await connection.fetchval('SELECT my_column FROM my_table WHERE something = $1', (arg,))

            await ctx.send(f'Returned {value}')
            at lconnection.close()

@bot.command()
async def establishCoreConfig(ctx):
    if ctx.author.id != ownerID:
        return
    await ctx.send("To confirm, you are going to reset the **entire** bot database.  Continue? (Yes/No)")
    import asyncio
    ...
    await connection.execute(""" CREATE TABLE tankDatabase (
    challengeID INT,
    originCategory TEXT,
    approvalStatus TEXT,
    weight FLOAT,
...
async def restartServer(ctx):
    ...
    try:
        ...
        if resp == "no":
            ...
            if connection:
                connection.close()
                await ctx.send("Database connection is closed")
...
async def stopServer(ctx):
    if ctx.author.id != ownerID:
        return
    await ctx.send("To confirm, the server will stop completely and require a manual reboot to turn back on.  \nInclude a backup?  Yes/No")
    import asyncio
    def check(m: discord.Message):
        return m.author.id == ctx.author.id and m.channel.id == ctx.channel.id
    ...
    try:
        ...
        if resp == "no":
            ...
            if connection:
                ...
                await ctx.send("Database connection is closed")
            # await backupFiles()
            await ctx.send("Backup successful.  Stopping now.")
            exit()
        else:
            await ctx.send("Cancelled.")
    except asyncio.TimeoutError:
...
bot.pool = loop.run_until_complete(asyncpg.create_pool(**SQLsettings))
