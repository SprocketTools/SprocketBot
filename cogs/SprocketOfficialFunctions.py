...
class SprocketOfficialFunctions(commands.Cog):
    ...
    async def askHamish(self, ctx: commands.Context):
        ...
        ai_response = await ctx.bot.AI.get_response(prompt=f"Analyze the attached message history and see if the developer 'Hamish' has already answered questions similar to the user's question.  They need to provide information that directly answers the user's question, not information that indirectly or tangentially answers the question.  If the correct information has been provided to directly answer the user's question, reply with a comma-separated list of the 'message_url' URLs that link to answers from Hamish.  If the user's question is a game suggestion, and not an more general question for the developer, reply with exactly 'no' so that the processing code allows the user to continue.  Otherwise, reply with exactly 'yes'", temperature=0.8, instructions=f"Here is the question asked by the user: {messageOut}\n----------------\nHere is the recent history in reverse order: {messages}\n\n")
