...
class ChannelConfig(commands.Cog):
    ...
    async def configureChannel(self, ctx: commands.Context, channel: discord.TextChannel = None):
        ...
        while True:
            ...
            embed.add_field(name="Automatic AI Interactions (coming soon)", value="Disabled" if len(await self.bot.sql.databaseFetchdictDynamic('''SELECT * FROM channel_blocks WHERE channel_id = $1 AND category = 'autoai';''', [target_channel.id])) > 0 else "Enabled", inline=False)
            embed.add_field(name="Manual AI Interactions (Jarvis, etc.)", value="Disabled" if len(await self.bot.sql.databaseFetchdictDynamic('''SELECT * FROM channel_blocks WHERE channel_id = $1 AND category = 'ai';''', [target_channel.id])) > 0 else "Enabled", inline=False)
            embed.add_field(name="Automatic Non-AI Fun Stuff (random -error posts, etc.)", value="Disabled" if len(await self.bot.sql.databaseFetchdictDynamic('''SELECT * FROM channel_blocks WHERE channel_id = $1 AND category = 'autoerror';''', [target_channel.id])) > 0 else "Enabled", inline=False)
            embed.add_field(name="Fun Commands (-addError, -getError, etc.)", value="Disabled" if len(await self.bot.sql.databaseFetchdictDynamic('''SELECT * FROM channel_blocks WHERE channel_id = $1 AND category = 'error';''', [target_channel.id])) > 0 else "Enabled", inline=False)
            embed.add_field(name="Utility (-weather, -submitDecal, etc.)", value="Disabled" if len(await self.bot.sql.databaseFetchdictDynamic('''SELECT * FROM channel_blocks WHERE channel_id = $1 AND category = 'utility';''', [target_channel.id])) > 0 else "Enabled", inline=False)
